...
The easiest way to explain this is with a simple example.  Suppose you work for Red-Bull Enterprises and develop an application with the following style sheet:
...
Now suppose Red-Bull Enterprises merges with VacaAzul and they want a 'themed' build of the site, but with 'blue' instead of 'red'.  Here's what you do:

- Copy/rename Widget.scss to _WidgetImpl.common.scss.  Remove the 'red' settings.
```javascript
.container {
    font-size: 20px;
}
```
- Create a new file called _WidgetImpl.red.scss.  The contents are listed below:
```javascript
@import "WidgetImpl.common"

.container {
    background-color: red;
}
```
- Create a new file called _WidgetImpl.blue.scss.  The contents are listed below:
```javascript
@import "WidgetImpl.common"
...
```
- Now change Widget.scss as follows:
```javascript
@import "WidgitImpl"  /* Won't this crash? */
```

When you perform a build and want 'red', set the themeMapper option to the following kind of function:
```javascript
module.exports = {
    ...
    module: {
        rules: [{
            ...
            use: [{
                ...
                loader: "sass-loader", options: {
                    sourceMap: true,
                    themeMapper: x => x.endsWith('Impl') ? x + '.red' : x;
                }
            }]
        }]
    }
...
Let's look at what really happens with Widget.scss
```javascript
@import "WidgitImpl"
 ...
 * This never runs.
 * 
 * When the themeMapper is [x => x.endsWith('Impl') ? x + '.red' : x], the following renders:
 * @import "WidgitImpl.red"
 * 
 * When the themeMapper is [x => x.endsWith('Impl') ? x + '.blue' : x], the following renders:
 * @import "WidgitImpl.blue"
 */
```
